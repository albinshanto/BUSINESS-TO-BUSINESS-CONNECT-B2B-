...
<div class="order-container">
  <table class="table-custom">
    ...
    <tbody>
      ...
      $s = "select * from retailer where rtid in (select uid from orderm where wid=".$_SESSION['uid'].")";
      $rs = $db->selectData($s);
      
      while ($row1 = mysqli_fetch_array($rs)) {
      ?>
      <tr>
        <td><span class="status-label"><?php echo $row1['name']; ?></span></td>
            ...
            <table class="table-custom">
              ...
              <?php
                ...
                $s1 = "select * from orderc 
                       ...
                       where orderm.wid=".$_SESSION['uid'];
                $rs1 = $db->selectData($s1);
                while ($row = mysqli_fetch_array($rs1)) {
                 // $tamt += ($row['amt'] * $row['qqty']);
                  $oid = $row['oid'];
                  $ostatus = $row['status'];
                  $pstatus = $row['pstatus'];
              ?>
  ...
  <?php if($ostatus=="requested"){ ?>
      ...
      <a href="acceptreq.php?id=<?php echo $oid;?>">Accept and Add Invoice</a>
